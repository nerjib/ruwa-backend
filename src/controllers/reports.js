...
//individual project report
  router.get('/project/:id', async (req, res) => {
    const getAllQ = 'SELECT  reports.activitydate,reports.date,projects.community,projects.facility,projects.lot,projects.pstatus,reports.id,reports.summaryfrom,summaryto,users.first_name,users.last_name from reports left join users on reports.uid=users.id left join projects on reports.pid=projects.id where reports.complete=$1 and reports.pid=$2 order by reports.id desc';
    try {
      // const { rows } = qr.query(getAllQ);
...
router.get('/activity/projectweekly/:id', async (req, res) => {
  const getAllQ = 'SELECT * FROM weeklyreports WHERE pid= $1';
  try {
    // const { rows } = qr.query(getAllQ);
    const { rows } = await db.query(getAllQ, [req.params.id]);
    return res.status(201).send(rows);
