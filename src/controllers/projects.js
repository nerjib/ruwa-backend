...
router.post('/', async (req, res) => {
  ...
  projects (title,state_id, local_id,location,lga,status,wardheadphone,gps,started,lot,type,phase,contractor_id,ward,community)
...
return res.status(400).send(error);
 ...
 lga=$7, contractor_id=$8, started=$9, finish=$10, ward=$11, facility=$12, community=$13, compartment=$14, pstatus=$15, lot=$16 WHERE id=$17 RETURNING *`;
  ...
  router.get('/completeprojects/all', async (req, res) => {
    const getAllQ = 'SELECT   projects.local_id,projects.state_id,projects.community,projects.facility,projects.lot,projects.pstatus,projects.id,projects.title,projects.lga,projects.ward,projects.status,contractors.company,users.first_name,users.last_name from Projects left join contractors on projects.contractor_id=contractors.id left join users on users.id=projects.state_id order by projects.lot asc, projects.id desc';
    try {
      // const { rows } = qr.query(getAllQ);
      const { rows } = await db.query(getAllQ);
      return res.status(201).send(rows);
